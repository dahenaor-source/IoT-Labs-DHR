...
| --- | --- | --- | --- |
| Task 1 | `firmware/lab0_http/prj.conf` | Activa Wi-Fi, servidor HTTP, JSON y LED strip | El firmware debe compilar sin el error de `TASK 1` |
| Task 2 | `firmware/lab0_http/src/main.c` (`led_set`) | Controla el LED RGB WS2812 integrado en GPIO8 | `POST /api/control` cambia el LED |
| Task 3 | `firmware/lab0_http/src/main.c` (`sensor_handler`) | Genera una temperatura simulada entre 20.0 y 29.9 °C | `GET /api/sensor` devuelve JSON |
| Task 4 | `firmware/lab0_http/src/main.c` (`control_handler`) | Recibe JSON, interpreta `state` y acciona el LED | `POST /api/control` devuelve `{"status": "ok"}` |
...
`firmware/lab0_http/boards/esp32c6_devkitc_hpcore.overlay`.
...
Primero editar `tools/dashboard_http.py` y cambiar:
