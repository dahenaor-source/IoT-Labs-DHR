 ...
 *
 * PUBLISH   iot/sensor  -> {"temperature": 24.5}
 * SUBSCRIBE iot/control <- {"state": 0|1}
 */
...
#error "TASK 1 is not done: enable Wi-Fi, MQTT, JSON and LED strip in prj.conf."
#endif

#if !defined(CONFIG_LAB_BROKER_ADDR) || !defined(CONFIG_LAB_BROKER_PORT)
#error "TASK 2 is not done: declare broker address and port in Kconfig."
#endif
				   ...
				   const struct mqtt_publish_param *pub)
{
	...
	if (pub->message.topic.qos == MQTT_QOS_1_AT_LEAST_ONCE) {
		...
		LOG_INF("PUBACK sent for message %u", pub->message_id);
	}
}
...
static void broker_init(void)
{
	...
	addr->sin_port = htons(CONFIG_LAB_BROKER_PORT);
	zsock_inet_pton(AF_INET, CONFIG_LAB_BROKER_ADDR, &addr->sin_addr);
}
...
int main(void)
{
	...
	while (1) {
		int rc;
		int64_t next_publish = k_uptime_get() + 2000;
